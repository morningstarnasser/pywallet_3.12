...
def _to_json_safe(obj):
    ...
    if isinstance(obj, dict):
        return {(_to_json_safe(k)): _to_json_safe(v) for k, v in obj.items()}
...
def read_wallet(json_db, db_env, walletfile, print_wallet, print_wallet_transactions, transaction_filter, include_balance, FillPool=False):
    ...
    def item_callback(type, d):
        if type == b"tx":
            ...
            compressed = d['public_key'][0] != 0x04
...
#        print(recoveredKeys[0:5])
    ...
    if options.dump:
        if options.dumpformat == 'addr':
            ...
            json_db = addrs
        _safe = _to_json_safe(json_db)
        wallet = json.dumps(_safe, sort_keys=True, indent=4, ensure_ascii=False)
        print(wallet)
        exit()
    elif options.key:
